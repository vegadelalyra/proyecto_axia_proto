...
class assetstatusController extends BaseController
{
    ...
    public function getAction($args, $callerInfo, $rawdata, $request, $verb) {
        if (isset($args[0]) && is_numeric($args[0]) && count($args) == 1 && empty($verb)) {
            $toReturn =  $this->proxy->genericGetById("assetstatus",$args[0],$callerInfo);
            if (empty($toReturn))
            ...
            if (count($args) == 0) {
                return $this->proxy->genericGetElements("assetstatus", isset($request["start"])?$request["start"]:null, isset($request["limit"])?$request["limit"]:BaseController::DEFAULT_LIMIT,
