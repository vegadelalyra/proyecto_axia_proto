...
class DocumentDBCollection
{
   ...
   */
  public function query($query, $partitionValue = null, $start = 0, $limit = 1000)
  {
    return $this->document_db->query($this->rid_db, $this->rid_col, $query, $partitionValue, $start, $limit);
  }
   ...
   * @return string JSON strings
   */
  public function createDocument($json, $partitionValue = null)
  {
    return $this->document_db->createDocument($this->rid_db, $this->rid_col, $json, $partitionValue);
  }
...
class DocumentDB
{
  ...
  private function getAuthHeaders($verb, $resource_type, $resource_id)
  {
    ...
    $master = 'master';
    $token = '1.0';
    $x_ms_version = '2016-07-11';//'2017-02-22';
   ...
   */
  private function request($path, $method, $headers, $body = NULL, $returnFull = false)
  {
   ...
   * @return string JSON response
   */
  public function query($rid_id, $rid_col, $query, $partitionValue = null, $start = 0, $limit = 1000)
  {
    //TODO: Cache of continue ID
    $page = 0;
    $result = null;
    if ($start % $limit !== 0)
      throw new Exception('Start must be a multiple of Limit');
    $page = $start/$limit;
    $continue = null;
    for ($i = 0; $i <= $page; $i++) {
      ...
      $headers[] = 'x-ms-max-item-count:'.$limit;
      //$headers[] = 'x-ms-documentdb-isquery:true';
      if (!empty($partitionValue))
        $headers[] = 'x-ms-documentdb-partitionkey: [ "'.$partitionValue.'" ]';
      if (!empty($continue))
        $headers[] = 'x-ms-continuation: '.$continue.'';
      //var_dump($headers);
      $jsonQuery = '{"query": "'.$query.'","parameters":[]}';
      $response = $this->request("/dbs/" . $rid_id . "/colls/" . $rid_col . "/docs", "POST", $headers, $jsonQuery,true);
      //var_dump($response);
      $result = $response->getBody();
      $continue = $response->getHeader('x-ms-continuation');
      //echo $i." ".$continue."\n";
      if (empty($continue) && $page > $i)
        return null;
    }
    return $result;
  }
   ...
   * @return string JSON response
   */
  public function createDocument($rid_id, $rid_col, $json, $partitionValue = null)
  {
    $headers = $this->getAuthHeaders('POST', 'docs', $rid_col);
    $headers[] = 'Content-Length:' . strlen($json);
    if (!empty($partitionValue))
